...
assert(hookNoEsClaim("CREADOR DE CONTENIDO Y COMUNICADOR", "Juan Pérez", "Creador de contenido y comunicador") !== null, "la credencial como titular falla");
assert(hookNoEsClaim("CREADOR DE CONTENIDO Y COMUNICADOR") !== null, "ocupación + complementos falla también sin credencial");
assert(hookNoEsClaim("COMO ESTUDIAR MEJOR") === null, "«COMO ESTUDIAR MEJOR» pasa");
assert(hookNoEsClaim("NUTRICIONISTA DESMIENTE EL 98%", "Ana Gómez", "Nutricionista deportiva") === null, "ocupación + verbo + dígito pasa");
assert(hookNoEsClaim("FUNDADOR QUEBRÓ TRES VECES", "Juan Pérez", "Fundador de X") === null, "ocupación + verbo pasa");
assert(hookNoEsClaim("ESPECIALISTA EN ORATORIA", "Ana Gómez", "Especialista en oratoria") !== null, "la credencial exacta falla");

// --- verificación de citas
const transcript = "[0:12] INVITADO — Yo hago entre 300 y 400 sentadillas por día, y lo tengo medido.";
...
const base: CopyPieces = {
  ...
  credencial: "Entrenador",
};
const p0 = findProblems(base, transcript, { invitado: "Juan Apud" });
assert(p0.length === 0, `una pieza correcta no dispara nada (${p0.join(" | ") || "sin problemas"})`);
const p1 = findProblems({ ...base, hook_edicion: "CON JUAN APUD" }, transcript, { invitado: "Juan Apud" });
assert(p1.some((x) => x.includes("sólo el nombre del invitado")), "findProblems marca el titular = nombre");
...
assert(captionSinElNumero(base, "Juan Apud") === null, "caption con «300 y 400» pasa");
assert(captionSinElNumero({ ...base, claim: "Nuevo episodio con Juan Apud, ya disponible" }, "Juan Apud") !== null, "teaser falla");
assert(captionSinElNumero({ ...base, claim: "La rutina de sentadillas que cambió todo" }, "Juan Apud") !== null, "claim sin el número del titular falla");
...
assert(captionSinElNumero({ ...base, molde: "cita", cita_textual: "«Yo hago entre 300 y 400 por día»", claim: "otra cosa" }, "Juan Apud") === null, "molde cita: la L1 es la cita");

// --- el armado del caption es código
const cap = buildCaption(base, "Juan Apud", "tiktok");
...
const capCita = buildCaption({ ...base, molde: "cita", cita_textual: "«Lo tengo medido»" }, "Juan Apud");
assert(capCita.startsWith('"Lo tengo medido" Juan Apud 🏋️'), `cita: comillas + nombre + emoji (${capCita.split("\n")[0]})`);
