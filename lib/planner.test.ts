...
import { cederLaSemana, episodeWeekStart, faltantesDeLaSemana, isoDow, nextFreeSlots, parseWeeklyPlan, redesDe, redNoAdmitida, weekSlots } from "./planner.ts";
...
const item = (id: string, episodeId: string, date: string, status: Item["status"] = "planned"): Item => ({
  id,
  episodeId,
  kind: "clip",
  date,
...
const state = (items: Item[]): State => ({ version: 1, episodes: [], items, log: [] });
...
assert(episodeWeekStart("2026-09-13T20:00:00Z", "2026-09-14") === "2026-09-13", "la semana del episodio arranca su domingo");
assert(episodeWeekStart("2026-08-30T20:00:00Z", "2026-09-14") === "2026-09-13", "un episodio viejo usa la semana en curso");
...
// cederLaSemana: lo de otro episodio se corre 7 días, lo publicado no se toca
...
// nextFreeSlots con semana extendida: día por medio
const ext = nextFreeSlots(state([]), "clip", "2026-09-14", 3, { stretch: true });
assert(ext.join(",") === "2026-09-14,2026-09-16,2026-09-18", `semana extendida: día por medio (${ext.join(",")})`);
