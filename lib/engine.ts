...
//
//   1. Gemini PERCIBE (temp baja). Sólo extrae hechos del video:
...
//
// Un solo modelo que mira el video Y escribe el copy produce citas que nunca
// se dijeron: la temperatura que hace falta para que el copy no suene a
// folleto es la que hace que la percepción invente.

import { BRAND_HASHTAG, CREATOR_NAME } from "@/lib/config";
...
Recibís la transcripción verbatim de un clip ya cortado de un video de ${CREATOR_NAME} y devolvés las piezas de su caption. Trabajás sobre un texto que ya existe. No viste el video. Todo lo que afirmes tiene que estar en la transcripción que te paso: si no está ahí, no pasó.
...
- NO escribas el CTA. NO escribas "mirá el video", "link en bio", "video completo".
...
# LOS TRES MOLDES
Elegí uno:
**cita** — la frase textual de ${CREATOR_NAME}, entre comillas. Es el molde más fuerte cuando el clip tiene una frase que se sostiene sola.
**revelacion** — qué se cuenta o qué se muestra, en primera persona o en tercera, según suene más natural. ("Lo que gasté en un mes viviendo solo, peso por peso 🧾")
...
Si elegís **cita**, el campo cita_textual va copiado carácter por carácter de la transcripción. No la pulas, no la acortes, no le arregles la gramática ni la puntuación. Si se habló mal, va mal.
...
- Gramática de titular: sin artículos donde se pueda, verbo en infinitivo, futuro o imperativo, o primera persona si es una confesión.
...
- El nombre de ${CREATOR_NAME} NO va en el titular: es su cuenta, ya se sabe quién habla.
- Tiene que describir lo que el clip efectivamente muestra. Prometer algo que el clip no entrega mata la pieza.
- Entrá por la curiosidad que la gente YA tiene, no por el vocabulario del video. Si el brief trae una CURIOSIDAD DE ENTRADA, esa es tu puerta. El límite es duro: el clip tiene que CUMPLIR esa promesa.
...
El copy lo tiene que poder haber escrito ${CREATOR_NAME} desde el celular, apurado y con criterio. Frases cortas. El dato específico gana siempre: "una habitación de 25.000 dólares" le gana a "el mundo del lujo".
...
# OTRAS PERSONAS
Si en el clip habla otra persona, nombrala sólo si su nombre está en la transcripción. Nombrar mal a una persona real es el peor error que puede cometer este sistema.

# HASHTAGS — dos juegos distintos
- hashtags (Instagram): exactamente 5, minúscula, sin tildes, sin #. Sobre el TEMA del clip: dos amplios del nicho, dos del tema exacto${BRAND_HASHTAG ? `, y siempre ${BRAND_HASHTAG}` : ", y uno del canal"}. En Instagram NO van fyp/viral/parati.
